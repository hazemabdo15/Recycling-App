...
export const orderService = {
    ...
    if (item.measurement_unit === 1) {
      ...
      throw new Error(`Item ${index + 1}: Invalid measurement_unit. Must be 1 (KG) or 2 (Pieces)`);
        ...
        if (response.status === 401) {
          ...
          console.error('[orders API] Create order error response:', errorText);
  ...
  async getOrders(token) {
    try {
      console.log('[orders API] Getting orders with token:', token ? 'Token provided' : 'No token');
      
      // For development, check if it's a mock token
      if (token && token.startsWith('mock.')) {
        console.log('[orders API] Mock token detected for get orders, returning empty orders list for development');
        
        return {
          success: true,
          count: 0,
          data: [],
          message: 'Development mode - no orders configured'
        };
      }
      ...
      console.log('[orders API] Get orders response status:', response.status);
